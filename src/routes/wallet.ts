...
import { sendRedemptionCodeEmail } from "../lib/email.js";
import { z } from "zod";
...
router.post("/redeem", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (requester) {
    await sendRedemptionCodeEmail(requester.email, requester.firstName, code.code, "Campaign Objective Wallet redemption", amount);
  }
